...
\section{Miniconda}
Miniconda è una distribuzione minimale di Conda, un sistema di gestione degli ambienti e dei pacchetti Python. È stato utilizzato per creare ambienti isolati e riproducibili, in cui installare versioni specifiche delle librerie necessarie, garantendo che le specifiche versioni di PyTorch, CUDA e altre librerie fossero compatibili fra loro, evitando in questo modo conflitti che avrebbero potuto compromettere l'addestramento del modello.
\begin{figure}[htbp]
  \centering
...
\section{PyTorch}
PyTorch è un framework open-source per il deep learning sviluppato da Meta AI. Esso fornisce un'interfaccia dinamica e intuitiva per la definizione e l'ottimizzazione delle reti neurali, ed è particolarmente apprezzato per il suo supporto nativo all’accelerazione tramite GPU \cite{paszke2019pytorch}.\\
...
Data l'enorme mole di dati e la complessità del modello utilizzato, l'accelerazione hardware è stata indispensabile per ridurre i tempi di addestramento, rendendo possibile l'esecuzione di più esperimenti in tempi alquanto ragionevoli.
...
\end{figure}
La libreria è organizzata in moduli che separano la logica di tracking, la gestione dei video e il calcolo delle metriche di valutazione.
...
La scelta di tale modello è motivata dall'ottimo compromesso tra velocità ed accuratezza in tempo reale, una caratteristica di non poco conto nell'analisi di grandi moli di dati astrofisici.

\subsection{Caratteristiche principali}
YOLOv7 è progettato per realizzare un pipeline \emph{end-to-end}, che riceve in input un’immagine e restituisce in output le bounding box e le classi associate agli oggetti rilevati. \\
Si tratta del cuore del sistema di rilevamento sviluppato nel presente lavoro di tesi, dopo esser stato riadattato per analizzare i magnetogrammi e identidicare con elevata velocità e precisione la posizione e le dimensioni delle regioni attive, che rappresentano il primo passo per il successivo tracciamento.\\
Le principali caratteristiche del modello includono:
\begin{itemize}
...
\end{itemize}
L'architettura del modello separa nettamente le configurazioni dei modelli (iperparametri e anchor) dai pesi pre-addestrati e dagli script di inferenza e training.
...
\end{itemize}
Pertanto, considerare i dati HARP come semplice oggetto della tesi sarebbe riduttivo: essi sono stati \textit{fonte di "verità"} per il progetto. Sono stati utilizzati per creare il dataset annotato, indispensabile per addestrare il modello YOLOv7 a riconoscere queste strutture automaticamente.
