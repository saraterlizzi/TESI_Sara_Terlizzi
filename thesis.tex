...
\usepackage{longtable} %per separare le tabelle su più pagine
...
% Configurazione numerazione dei listing (dopo \lstset)
\AtBeginDocument{%
...
\Titolo{IDENTIFICAZIONE E TRACCIAMENTO DELLE REGIONI ATTIVE SUL SOLE}
...
\begin{document}
    ...
    \input{Chapters/2_tecnologie}
    \input{Chapters/3_applicazione}
    \input{Chapters/4_risultati}
    \input{Chapters/5_conclusioni}
    % --- INIZIO APPENDICE ---
    \cleardoublepage
    \appendix % Dice a LaTeX che da qui iniziano le lettere A, B, C...
    \input{Chapters/appendice} % Carica il file fisico
    % --- FINE APPENDICE ---
    %per bibliografia in ordine alfabetico
